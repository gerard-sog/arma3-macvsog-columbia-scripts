...
// a bush.
// *******************************************************
private _top_of_rope = "Sign_Sphere100cm_F" createVehicle [10, 10000, 0];
_top_of_rope setObjectMaterialGlobal [0, "\a3\data_f\default.rvmat"]; // makes sphere no longer see thru
_top_of_rope setObjectTextureGlobal [0, 'vn\characters_f_vietnam\opfor\uniforms\data\vn_o_nva_army_bdu_shirt_03_co.paa'];
_top_of_rope setPos (getPos _wire_trap vectorAdd [0,0,11.0]);
_top_of_rope setDir (_swing_dir + 180);

// *******************************************************
// Create sphere used later to hide mace uav.  And camouflage the sphere.
// *******************************************************
private _mace_start_pos = ([_top_of_rope, 8, (_swing_dir + 180)] call BIS_fnc_relPos);
private _mace_sphere = "Sign_Sphere100cm_F" createVehicle [10,10000,0];
_mace_sphere setPos [_mace_start_pos#0, _mace_start_pos#1, 9.0];
_mace_sphere setObjectMaterialGlobal [0, "\a3\data_f\default.rvmat"]; // makes sphere no longer transparent
...
// *******************************************************
private _mace_start_tree_pos = ([_top_of_rope, 12, (_swing_dir + 180)] call BIS_fnc_relPos);
private _tree_2 = createSimpleObject ["vn\vn_vegetation_f_enoch\tree\vn_t_fagussylvatica_1fc.p3d", [0, 0, 0]];
_tree_2 setPosATL [_mace_start_tree_pos#0, _mace_start_tree_pos#1, -6];
_tree_2 enableSimulation false;
...
// *******************************************************
 private _rope_top_obj = createVehicle ["B_UGV_02_Science_F", [20, 20, 0], [], 0, "CAN_COLLIDE"];
_rope_top_obj allowDamage false;
_rope_top_obj enableCollisionWith _tree;
_rope_top_obj setFuel 0;
_rope_top_obj engineOn false;
_rope_top_obj disableAI "ALL";
...
_bush enableCollisionWith _rope_top_obj;
_bush enableCollisionWith _tree;
_bush attachTo [_rope_top_obj, [0, 0, 0]];
_rope_top_obj attachTo [_top_of_rope, [0, 0, 0]];
_rope_top_obj allowDamage false;
_dir_to = ([_mace_sphere, _rope_top_obj] call BIS_fnc_dirTo);
_rope_top_obj setDir _dir_to;
...
_mace setVariable ["COLSOG_victim_animation_already_used", [], true]; // used to ensure different impale anim applied to many dudes impaled on same mace
_mace disableAI "ALL";
...
_mace enableCollisionWith _tree_2;
...
[[_wire_trap, _mace, _rope_top_obj, _mace_sphere], "functions\TRAPS\swinging\columbia_fnc_release_swinging_mace_trap.sqf"] remoteExec ["execVM", 0, true];
